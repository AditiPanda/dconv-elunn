...
class DnCNN(object):
    def __init__(self, sess, patch_size=40, batch_size=128,
                 ...
                 test_save_dir='./test',
                 dataset='BSD400', testset='Set12', load_flag=True, initial_epoch=0): # test set changed on 12-11-17
#        tf.reset_default_graph() 
        self.sess = sess
...
#            self.saver = tf.train.import_meta_graph('DnCNN.model-{}.meta'.format(last_epoch))
...
#            layer_16_output = self.layer(layer_15_output, [3, 3, 64, 64], d_rate=2)
            # layer 17
        with tf.variable_scope('conv5'):
            self.Y = self.layer(layer_4_output, [3, 3, 64, self.output_c_dim], useBN=False, useELU=False)   
...
#            layer_16_output = self.layer(layer_15_output, [3, 3, 64, 64], d_rate=2)
        # layer 17
        with tf.variable_scope('conv5', reuse=True):
            self.Y_test = self.layer(layer_4_output, [3, 3, 64, self.output_c_dim], useBN=False, useELU=False)
