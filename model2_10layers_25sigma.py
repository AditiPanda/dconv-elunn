...
#            self.sess.run(self.init)
            ...
            ################### commented on 1st Dec, 2017:  no need when create_vars is there; 
...
#            self.saver = tf.train.import_meta_graph('DnCNN.model-{}.meta'.format(last_epoch)) # commented on 1st Dec, 2017
...
#        max_to_keep added on 4th Dec, 2017
...
#            self.evaluate(epoch, test_data)  # test_data value range is 0-255 
    ...
    def sampler(self, image):
        ...
        print(self.X_test.get_shape().as_list())
...
#        test_data = load_images(test_files)  # list of array of different size, range 0-255
    ...
    def evaluate(self, epoch, test_data):
        ...
        for idx in xrange(len(test_data)):
            ...
            psnr = cal_psnr(groundtruth, outputimage)
            psnr_sum += psnr
            save_images(groundtruth, noisyimage, outputimage, os.path.join(self.sample_dir, 'test%d_%d.png' % (idx, epoch)))
